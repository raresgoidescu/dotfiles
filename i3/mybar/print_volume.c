...
void print_volume() {
	...
	int res = snd_mixer_selem_get_playback_dB_range(elem, &min, &max) || snd_mixer_selem_get_playback_dB(elem, 0, &val);
	if (res || min >= max) {
		res = snd_mixer_selem_get_playback_volume_range(elem, &min, &max);
		res |= snd_mixer_selem_get_playback_volume(elem, 0, &val);
		force_linear = 1;
	}
	...
	if (snd_mixer_selem_get_playback_switch(elem, 0, &pbval) < 0) goto volume_err_late;
